...
const getOrder = async (orderID: string | undefined) => {
  const res = await fetch(
    `${process.env.API_URI}/api/v1/order/detail/${orderID}`,
    { cache: "no-cache" }
  );
  return res.json();
};
  ...
  return (
    <main className="container mx-auto xl:px-20 duration-300 mb-20">
      <h1 className="font-medium text-2xl text-center">
        Order Details - {order.orderId ?? order.referenceID}
      </h1>
      ...
      <div className="flex items-center flex-col justify-center relative">
        ...
        <ul className="flex justify-between items-center w-full px-7 my-2">
          <li
            className={`flex items-center w-full after:w-full 
          after:content-[""] after:border-4 ${
            order.confirmedDate ? "after:border-primary" : "after:border-gray"
          } after:items-center`}
          >
          ...
          {order.status !== "Declined" && (
            <li
              className={`flex items-center w-full after:w-full 
          after:content-[""] after:border-4 ${
            order.completedDate ? "after:border-primary" : "after:border-gray"
          } after:items-center`}
            >
              <span className="text-white bg-primary p-4 rounded-full flex items-center justify-center">
          ...
          <li className="flex items-center">
            <span
              className={`text-white ${
                order.status === "Declined" ? "bg-red-500" : "bg-primary"
        ...
        <div className="flex w-full justify-between ">
          ...
          {order.status !== "Declined" && (
            <div className="flex flex-col items-center text-center w-32 ">
              {order.confirmedDate ? (
                <>
                  <span className="text-xs text-gray-500">
                    {getFormattedDate(order.confirmedDate)}
                  </span>
          ...
          <div className="flex flex-col items-center w-32 ">
            {order.completedDate ? (
              <>
                <span className="text-xs text-gray-500">
                  {getFormattedDate(order.completedDate)}
                </span>
