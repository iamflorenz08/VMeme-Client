        ...
        <tbody className="text-center overflow-auto">
          {orders.map((order, index) => (
            <tr key={index} className="odd:bg-white even:bg-primary-100">
              <td className="py-4">{order.orderId ?? order._id}</td>
              <td className="py-4">
                {order.orderedPaintings
                  .slice(0, 3)
                  .map((painting: IPainting, index: number) =>
                    index >= order.orderedPaintings.length - 1
                      ? painting.name
                      : painting.name + ", "
                  )}
                {order.orderedPaintings.length > 3 && "..."}
              </td>
              <td className="py-4">GCash</td>
